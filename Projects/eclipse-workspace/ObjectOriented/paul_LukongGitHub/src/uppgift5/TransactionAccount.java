...
public class TransactionAccount extends Main {
	...
	public static GridPane account(Stage stage) {
		...
		TableColumn<Kunder, String> column1 = new TableColumn<>("Spara");
		column1.setCellValueFactory(new PropertyValueFactory<Kunder, String>("spar"));
		column1.prefWidthProperty().bind(table.widthProperty().multiply(.2));

		TableColumn<Kunder, String> column2 = new TableColumn<>("Uttag");
		column2.setCellValueFactory(new PropertyValueFactory<Kunder, String>("uttag"));
		column2.prefWidthProperty().bind(table.widthProperty().multiply(.2));

		TableColumn<Kunder, String> column3 = new TableColumn<>("OutGoingBalance");
		column3.setCellValueFactory(new PropertyValueFactory<Kunder, String>("balans"));
		column3.prefWidthProperty().bind(table.widthProperty().multiply(.2));

		TableColumn<Kunder, String> column4 = new TableColumn<>("IncomingBalance");
		column4.setCellValueFactory(new PropertyValueFactory<Kunder, String>("balans1"));
		column4.prefWidthProperty().bind(table.widthProperty().multiply(.2));

		TableColumn<Kunder, String> column5 = new TableColumn<>("Date");
		column5.setCellValueFactory(new PropertyValueFactory<Kunder, String>("date"));
		...
		add.setOnAction(e -> {
			...
			if (balans3 < 0) {
				...
				table.getItems().add(customer = new Kunder(text.getText(), firstText.getText(), secondText.getText(),
						thirdText.getText(), LocalDateTime.now()));
