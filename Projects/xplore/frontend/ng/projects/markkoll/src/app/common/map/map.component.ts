...
import { defaults, DragAndDrop, Interaction, MouseWheelZoom, Select } from "ol/interaction";
...
/**
 * MkMapComponent används för att visa kartor i Markkoll.
 * Komponenten har minimalt med beroenden till Xplores karttjänster pga att de
 * för närvarande inte stödjer flera kartinstanser samtidigt.
 * Bakgrundskartor från Metria Maps hämtas antingen konfigurerade för kund i KundConfig
 * eller som fördefinierade kartor i klienten.
 */
@Component({
  selector: "mk-map",
...
export class MkMapComponent implements OnInit, AfterViewInit {
  id: string;
  map: Map;
  mapView: View;
   ...
   */
  @Output() infoClick = new EventEmitter<FeatureInfoResult[]>();

  /**
    * Skickar ett event när geometrier valts i kartan.
    */
  @Output() selectionChange = new EventEmitter<FeatureSelectedResult[]>();

  /**
   * Skickar ett event när features laddats från fil.
   */
  @Output() featuresLoaded = new EventEmitter<number>();
  ...
  constructor(
    ...
    this.fullscreen = transloco.translate("mk.karta.fullscreen");
    this.leaveFullscreen = transloco.translate("mk.karta.leaveFullscreen");
    this.mapConfig = this.configService.config;
    this.id = "map-" + this.generateId();
  ...
  private initMap() {
    ...
    // Hämta tumnagel med bakgrundskartor.
    if (!this.isStatic) {
      const coord = this.fitToExtent ? getCenter(this.extent) : this.configService.config.thumbnailCoordinates;
      const resolution = 1;

      this.backgroundLayers.forEach(layer => {
        const tileUrl = this.getTileUrl(this.getLayer(layer.getProperties().id), coord, resolution);

        this.http.get(tileUrl, { responseType: "blob" }).pipe(
          map(blob => this.domSanitizer.bypassSecurityTrustUrl(URL.createObjectURL(blob))))
          .subscribe(url => {
            this.mapThumbnails.push({ id: layer.getProperties().id, title: layer.getProperties().title, url: url });
          });
      });
    }
    ...
    this.mapSelectInteraction.on("select", evt => {
      ...
      this.mapSelectInteraction.getFeatures().forEach(feature => {
        ...
        const layerId = this.mapSelectInteraction.getLayer(feature)?.get("id") || "unknown";
      ...
      this.selectionChange.emit(Object.values(selectionResult));
    });
    ...
    if (this.allowFileDropImport) {
      ...
      this.dragAndDropInteraction.on("addfeatures", (event) => {
        ...
        this.map.getView().fit(this.importedFeatureVectorSource.getExtent(), { padding: this.paddingInMap });
        this.map.addLayer(
          this.importedFeatureVectorLayer
        );
        this.featuresLoaded.emit(this.importedFeatures.length);
        this.fileLoaded.emit(<any>event.file.name);
      });
  ...
  private handleInfoClickInMap(event: ClickEvent): void {
    ...
    forkJoin(infoCalls).pipe(
      map(results => {
        this.infoClick.emit(results);
      })
    ).subscribe();
  }

  private getWMSFeatureInfo$(layer: BaseLayer, event: ClickEvent): Observable<FeatureInfoResult> {
    ...
    const url = source.getFeatureInfoUrl(event.coordinate, event.resolution, event.projection, queryOpts);
  ...
  getLayer(layerId): Layer {
    return <Layer>this.mapLayers.getLayers().getArray().find(layer => layer.get("id") === layerId);
  }

  zoomToFeatures(features: Feature[]) {
    const geometries = new GeometryCollection(features.map(f => f.getGeometry()));
  ...
  switchBackgroundLayer(layerId: string) {
    ...
    const selectedBackgroundLayer = this.backgroundLayers.find(layer => layer.getProperties().id === layerId);
  ...
  */
  setLayerVisibility(layerId: string, visible: boolean) {
    const selectedLayer = this.layers.find(layer => layer.getProperties().id === layerId);
