...
@Component({
  selector: "xp-search-ort",
  template: `
    <xp-search-field #searchField placeholder="Ange ortnamn" [options]="options | async" (placeSelected)="onPlaceSelected($event)"></xp-search-field>
  `
})
export class SearchOrtComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.options = this.searchField.textInput.pipe(
      flatMap(query => query.length > 2 ? this.sokService.findOrter(query, this.mapService.getLocation()).pipe(
        catchError(() => of([]))
      ) : of([]))
    );
  }
