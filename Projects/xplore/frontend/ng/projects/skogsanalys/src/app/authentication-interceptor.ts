...
import {AuthConfig, OAuthErrorEvent, OAuthModuleConfig, OAuthService, OAuthStorage} from "angular-oauth2-oidc";
                  ...
                  if (refreshError instanceof HttpErrorResponse) {
                    if (refreshError.error && refreshError.error.error && refreshError.error.error === "invalid_grant") {
                      if (!window.location.pathname.startsWith("/oauthLogin")) {
                        sessionStorage.setItem(RETURN_URL_PROPERTY, window.location.pathname + window.location.search);
                      }
