...
@Component({
  ...
  selector: "xp-avverkningsstatistik-report",
  template: `
    <p>
      Redovisning av det valda områdets påverkan av slutavverkningar. Informationen kommer från Skogsstyrelsens data
      ”Utförda avverkningar” som skapas med hjälp av satellitbildsanalyser. Statistiken gäller slutavverkningar och
      inte gallringar. Områden mindre än 0,5 hektar finns inte med i informationen.
    </p>
    <br/>
...
export class AvverkningsstatistikReportComponent implements OnChanges {
  ...
  updateChartData() {
    ...
    const minNumberOfIntervals = 0; // The minimum number of intervals that are always shown in the chart
    const intervalLength = 1; // The length of each interval in years
    ...
    for (let i = 0; i < avverkningar.length; i++) {
      ...
      if (!intervalTotalAreal[intervalIndex]) {
        intervalTotalAreal[intervalIndex] = 0;
      }
    ...
    for (let i = 0; i < intervalTotalAreal.length; i++) {
      ...
      this.chartData[i] = {
        "name": intervalStart === intervalEnd ? intervalStart.toString() : intervalStart + "-" + intervalEnd,
