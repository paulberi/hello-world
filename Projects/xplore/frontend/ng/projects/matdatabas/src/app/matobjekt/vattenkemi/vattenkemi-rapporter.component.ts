...
@Component({
  selector: "mdb-vattenkemi-rapporter",
  template: `
    <div class="main-content" [ngStyle]="{'display': error !== RestError.GET_RAPPORTER ? 'grid': 'none'}">
      <table mat-table [dataSource]="dataSource" matSort matSortActive="skapad_datum" matSortDisableClear
        ...
        <ng-container matColumnDef="filnamn">
          <th mat-header-cell *matHeaderCellDef mat-sort-header disableClear>Filnamn</th>
        ...
        <ng-container matColumnDef="remove">
          ...
          <td mat-cell *matCellDef="let d">
            <button mat-icon-button (click)="onTaBort(d.id)">
      ...
      <div class="actions">
        <button type="button" (click)="onLaggTill()" mat-raised-button color="primary" [disabled]="showDonut">
          <span *ngIf="!showDonut">Lägg till rapport</span>
          <mat-spinner *ngIf="showDonut" [diameter]="20" color="accent"></mat-spinner>
        </button>
      </div>
    </div>
    <p *ngIf="error" class="rest-error">{{error}}</p>
    <input id="rapportFileInput" hidden type="file" (change)="onSelected($event)" name="file">
  `,
  styles: [`
    .main-content {
      display: grid;
...
export class VattenkemiRapporterComponent implements OnInit {
  ...
  updateDokument() {
    ...
    this.bifogadFilService.getList(this.ids).subscribe(
      ...
      error => {
        this.error = RestError.GET_RAPPORTER;
        this.uploading.emit(false);
      });
  }
  ...
  onLaggTill() {
    this.error = null;
    document.getElementById("rapportFileInput").click();
  }
  ...
  onSelected(event) {
    ...
    this.bifogadFilService.post(event.target.files[0]).subscribe(
      ...
      () => {
        this.error = RestError.POST_RAPPORT;
        this.uploading.emit(false);
      });
  }
  ...
  onTaBort(id: string) {
    ...
    this.dialogService.showConfirmationDialog(
      new ConfirmationDialogModel(
        "Ta bort",
        `<p>Är du säker på att du vill ta bort rapporten '${filename}'?<p>Operationen går inte att ångra efter att man tryckt på Spara.`,
        "Avbryt",
        "Ta bort"),
...
export enum RestError {
  GET_RAPPORTER = "Misslyckades hämta analysrapporterna för mätobjektet. Ladda om sidan för att försöka på nytt.",
