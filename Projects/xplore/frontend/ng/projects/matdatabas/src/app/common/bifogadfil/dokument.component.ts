...
export enum RestError {
  GET_DOKUMENT = "Misslyckades hämta dokumenten. Ladda om sidan för att försöka på nytt.",
  POST_DOKUMENT = "Misslyckades lagra dokumentet. Försök igen om en liten stund"
}

@Component({
  selector: "mdb-dokument",
  template: `
    <div class="main-content" [ngStyle]="{'display': error !== RestError.GET_DOKUMENT ? 'grid': 'none'}">
      <table mat-table [dataSource]="dataSource" matSort matSortActive="skapad_datum" matSortDisableClear
        ...
        <ng-container matColumnDef="remove">
          ...
          <td mat-cell *matCellDef="let d">
            <button *ngIf="!readonly" mat-icon-button (click)="onTaBort(d.id)">
      ...
      <div *ngIf="!readonly" class="actions">
        <button type="button" (click)="onLaggTill()" mat-raised-button color="primary" [disabled]="showDonut">
          <span *ngIf="!showDonut">Lägg till {{laggTillText}}</span>
          <mat-spinner *ngIf="showDonut" [diameter]="20" color="accent"></mat-spinner>
        </button>
      </div>
    </div>
    <p *ngIf="error" class="rest-error">{{error}}</p>
    <input id="dokumentFileInput" accept="{{accept}}" hidden type="file" (change)="onSelected($event)" name="file">
  `,
  styles: [`
    .main-content {
      display: grid;
...
export class DokumentComponent implements OnInit {
  ...
  @Input() ids: number[];
  @Input() accept: string;
  @Input() laggTillText: string;
  @Input() acceptFunc: (file: File) => boolean;
  ...
  onSelected(event) {
    if (this.acceptFunc) {
      if (!this.acceptFunc.apply(this, [event.target.files[0]])) {
        return;
      }
    }
    this.uploading.emit(true);
  ...
  formatTimestamp(ts: string): string {
    return ts.replace("T", " ").substring(0, ts.lastIndexOf(":"));
  }
