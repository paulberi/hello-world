# Only for GeoVis Vy Vattenfall. Selects onlys from database table omradespolygon.
...
if os.path.exists("GeneratedAppConfig_Vattenfall.json"):
  answer = input('The file "GeneratedAppConfig_Vattenfall.json" already exists, overwrite? [y/n]: ')
  if answer == 'y':
    
    f = open("GeneratedAppConfig_Vattenfall.json", "w")
  else:
    print("Interupted!")
    exit()
else:
...
# ---------- Create layers för each gruppniva3 from database ----------
    ...
    lista_alla_3_nivaer = cursor.fetchall()
