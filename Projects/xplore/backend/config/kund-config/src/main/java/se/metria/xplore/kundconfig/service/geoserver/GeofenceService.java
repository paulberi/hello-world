...
import se.metria.xplore.kundconfig.service.geoserver.model.*;
...
public class GeofenceService {
    ...
    // Den berömda OneCo-fixen
    private static final String ONECO_ORG_NR = "556670-2147";
    private static final String ONECO_PROJEKT_ID = "efad7915-d3c1-4b04-afbd-b60df3b14e53";
    ...
    private Integer createRuleForAdmin(Integer prio) throws JsonProcessingException {
        var rule = buildBasicRule("ADMIN", Access.ALLOW, prio);
    ...
    private List<Integer> createRulesForKund(String kundId, Integer prio) throws JsonProcessingException {
        ...
        for (var workspace: workspaces) {
            var ftypes = geoserverService.getFeatureTypes(workspace);
            ...
            var layerGroups = geoserverService.getLayerGroups(workspace);
    ...
    private Rule buildLayerRule(String roleName, Boolean external, String workspace, String layerName, LayerType layerType, Access access, Integer priority) {
        ...
        if (layerType == LayerType.VECTOR) {
            var cqlFilter = String.format("kund_id='%s'", roleName);

            // OneCo-fixen, ska tas bort ASAP!
            if (roleName.equals(ONECO_ORG_NR) && external) {
                cqlFilter = String.format("%s AND projekt_id <> '%s'", cqlFilter, ONECO_PROJEKT_ID);
            }
            layerDetails.setCqlFilterRead(cqlFilter);
        }
